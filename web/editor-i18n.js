(function initMaweI18n(global) {
  ...
  const EN_TEXT = {
    ...
    '取消黏连联动': 'disable linked-boundary ripple',
    '取消黏连联动）': 'to disable linked-boundary ripple)',
    '将字幕起点贴到前一条结尾': 'Snap the subtitle start to the previous end',
    '将字幕终点贴到后一条开头': 'Snap the subtitle end to the next start',
    '注：微调幅度可在「设置->通用操作」中调节，默认 50ms': 'Note: Adjust the fine-tuning amount in “Settings -> General”; the default is 50 ms',
  ...
  const EN_ATTR = {
    ...
    '选中字幕后，方向键和按住字幕块/边界时的 A/D 每次调整的毫秒数': 'Milliseconds adjusted per arrow-key press or A/D press while holding a subtitle block or edge',
    '关闭（Esc）': 'Close (Esc)',
